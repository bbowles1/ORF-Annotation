...
def create_catalog(McGillivray_path, gff3_path, FASTA_path, outdir, refseq_only, refseq_path):
    ...
    # output catalog path
    outpath = outdir + 'uorf_catalog.tsv'
    
    # output bedpath
    bedpath = outdir + 'uorf.bed'
    
    # read in McGillivray et al dataframe
    ref_df = pd.read_csv(McGillivray_path, sep = '\t', delimiter = '\t', header = 0, names = ['uORF_ID', 'uORF_score', 'start_codon', 'chromosome', 'strand', 'START', 'STOP', 'gene', 'additional_transcripts','type', 'peptide_score'],
    ...
    ensg_df = pd.read_csv('/home/mayo/m181414/research/uorf_project/bior_script/Ensembl_GRCH37.87.gff3', header=0, sep='\t', comment='#', names=ensg_names, dtype = ensg_dtypes)
    ...
    ref_df = ref_df[[ 'chromosome', 'START', 'STOP', 'strand', 'uORF_ID', 'type', 'gene', 'gene_start', 'associated_uorfs',
     'additional_transcripts', 'peptide_score', 'start_codon', 'uORF_score', 'pli_score', 'OMIM', 'lengthflag']]
...
def create_range_df(gff3_path, FASTA_path, outdir):
    ...
    range_df.to_pickle(outdir + 'range_df.pkl')
...
def create_TITER_df(gff3_path, FASTA_path, outdir):
    ...
    CDS_df.to_csv(outdir + 'TITER_CDS.tsv', sep = '\t', index=False)
